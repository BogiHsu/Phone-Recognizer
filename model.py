...
def phone_recognizer(x, phone_num, batch_size, layer_num, layer_dim, dr = 0.5):
	...
	biases = tf.Variable(tf.constant(0.1, shape = [phone_num, ]))
	layers = []
	for i in range(layer_num):
		lstm_cell = rnn.BasicLSTMCell(layer_dim[i], forget_bias = 1.0)
		dr_cell = rnn.DropoutWrapper(cell = lstm_cell, input_keep_prob = 1.0, output_keep_prob = 1-dr)
		layers += [dr_cell]
